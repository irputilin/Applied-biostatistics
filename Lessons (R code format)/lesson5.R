...
## Нотация ...$p.value показывает, что значения p-value существуют внутри 
## объекта (результата выполнения функции) самостоятельно и к ним можно
## обращаться напрямую.
...
###### ДОМАШНЕЕ ЗАДАНИЕ ###### 
# 1. Используя функцию describe (пакет psych) сформировать таблицы с описательной статистикой
#    всех столбцов в ваших таблицах по заданным группам (используйте цикл for). Если групповой
#    переменной нет (либо уровней фактора меньше 3) - создайте её самостоятельно (сut).
# 2. Напишите функцию, которая по p-value делает вывод о нормальности/не нормальности 
#    распределения и выводит t-значение для t-критерия и X-squared значение для критерия
#    Харке-Бэра.

#### HINT ####
# Попробуйте записать результат выполнения критерия в переменную и посмотрите на то, что в этой 
# переменной хранится. Можете просто к названию переменной с результатом приписать символ $ и 
# посмотреть, что будет в выпадающем окне
